...
menu() {
    ...
    red "=================================="
    echo "                           "
    yellow "1. 安装Acme.sh域名证书申请脚本（必须安装）"
    yellow "2. 申请单域名证书（80端口申请,需要先完成DNS解析）"
    yellow "3. 申请单域名证书（CloudFlare API申请）（无需解析）"
    yellow "4. 申请泛域名证书（CloudFlare API申请）（无需解析）"
    yellow "5. 撤销并删除已申请的证书"
    yellow "6. 手动续期域名证书"
    yellow "7. 卸载Acme.sh域名证书申请脚本"
    yellow "0. 退出"
    echo "         "
